...
from utils.image import wavelet_reconstruction, adaptive_instance_normalization
...
class SpacedSampler:
    ...
    def sample_with_mixdiff(
        ...
        for i, step in enumerate(iterator):
            ...
            tiles_iterator = tqdm(
                _sliding_windows(h, w, tile_size // 8, tile_stride // 8)
            )
            for hi, hi_end, wi, wi_end in tiles_iterator:
                tiles_iterator.set_description(
                    f"Process tile with location ({hi} {hi_end}) ({wi} {wi_end})"
                )
                # noisy latent of this diffusion process (tile) at this step
                tile_img = img[:, :, hi:hi_end, wi:wi_end]
                # prepare condition for this tile
                tile_cond_img = cond_sr_img[
                    :, :, hi * 8 : hi_end * 8, wi * 8 : wi_end * 8
                ]
                tile_cond_ref_img = cond_ref_img[
                    :, :, hi * 8 : hi_end * 8, wi * 8 : wi_end * 8
                ]
                tile_cond_latent = self.model.apply_cond_ref_encoder(
                    tile_cond_img, tile_cond_ref_img
                )
                ...
                tile_noise = self.predict_noise(
                    tile_img, ts, tile_cond, cfg_scale, tile_uncond
                )
        ...
        for hi, hi_end, wi, wi_end in _sliding_windows(
            h, w, tile_size // 8, tile_stride // 8
        ):
            tile_img = img[:, :, hi:hi_end, wi:wi_end]
            ...
            if color_fix_type == "adain":
                tile_img_pixel = adaptive_instance_normalization(
                    tile_img_pixel, tile_cond_img
                )
                ...
                assert (
                    color_fix_type == "none"
                ), f"unexpected color fix type: {color_fix_type}"
    ...
    def sample(
        ...
        print("img_pixel:", img_pixel.shape)
